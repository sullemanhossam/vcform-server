...
app.get('/', async (req: Request, res: Response) => {
    ...
    const { fileContent } = req.query; // Expecting fileContent in the body

    if (!fileContent) {
      return res.status(400).json({ message: 'Missing fileContent in the request body' });
    }

  try {
    const data = await resend.emails.send({
      from: 'humanresources@resend.dev',
